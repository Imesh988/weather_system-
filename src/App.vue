<template>
  ...
  <div>
    ...
    <!-- AnimatedHero තවමත් තබා ගැනීමට අවශ්‍ය නම් මෙය තබන්න, 
         නැත්නම් PlanetTour පමණක් තබන්න. -->
   <PlanetTour />
    
    <!-- PlanetTour component එක මෙතැනට import කරන්න -->
   
    
    <div id="app" class="relative z-10 min-h-screen py-10 px-4 md:px-8 font-sans text-slate-900">
      <header class="max-w-[1200px] mx-auto mb-10 text-center animate-fade-in">
      ...
      <main class="max-w-[1200px] mx-auto space-y-10">
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-stretch animate-slide-up">
          <!-- Globe Card -->
          <div class="lg:w-3/5 bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl shadow-blue-100/50 border border-blue-50/50 overflow-hidden group hover:shadow-2xl transition-all duration-500">
            ...
            <WorldGlobe @country-selected="onGlobeCountrySelected" class="w-full h-full min-h-[400px] transform group-hover:scale-[1.02] transition-transform duration-700" />
          </div>
          ...
          <div class="lg:w-2/5 flex">
            <div v-if="loading" class="w-full bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-blue-50/50 p-10 flex flex-col items-center justify-center text-center">
              ...
              <p class="text-slate-500 font-medium animate-pulse">Syncing with NASA Satellites...</p>
            </div>
        ...
        <!-- Control Panel -->
        <div class="bg-white/80 backdrop-blur-md sticky top-5 z-10 p-5 rounded-2xl shadow-lg border border-white/50 flex flex-wrap gap-6 items-center justify-between animate-fade-in">
        ...
        <div v-if="!loading && currentWeather && historicalData" class="space-y-10 pb-20">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-slide-up" style="animation-delay: 0.2s">
            <div class="bg-white/90 backdrop-blur-sm p-6 rounded-3xl shadow-xl border border-slate-50/50 hover:border-blue-200 transition-colors">
        ...
        <!-- Error Handling -->
        <div v-if="!loading && errorMessage" class="max-w-md mx-auto mt-10 bg-red-50/90 backdrop-blur-sm border-l-4 border-red-500 p-6 rounded-2xl shadow-lg flex items-center gap-4 animate-bounce">
...
// AnimatedHero අවශ්‍ය නම් import කරන්න, නැත්නම් ඉවත් කරන්න.
